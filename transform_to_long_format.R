...
### === read data === ###
forest_fun_syn_wide <- read.table("BE_synthesis_forest_dat_wide_April2024_1.txt", header = T, sep = "\t")
forest_fun_helper <- read.table("forest_functions_helper.txt", header = T, sep = "\t")
### ===== ###

### === dataset transformation to long format === ###
#the final forest functions synthesis dataset will have the seven identifier columns:
...
long_dat <- forest_fun_syn_wide %>% 
  ...
  pivot_longer( cols = c(3:ncol(.)), names_to = c("Function_Name_Year"), 
                values_to = "Value")
...
long_dat.2 <- long_dat.1 %>% 
  subset( select = -c(Function_Name_Year)) %>% 
  relocate( Plot, Plot_bexis, Function_Name, Value, codedYear, dataID, Dataset_Version)

#fourth, rename columns for the upload on BExIS
names(long_dat.2)[names(long_dat.2) == "Function_Name"] <- "Function"
...
write.table(long_dat.2, file = "synthesis_dataset_functions_forest_long_version0.1.txt", quote = F, sep = "\t", row.names = F)
### ===== ###
